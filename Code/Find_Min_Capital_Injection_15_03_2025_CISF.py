...
import Data_09_03_2025_CISF  # Updated data module
import matplotlib.pyplot as plt
import os
import glob

# Ensure the "plots_capital_injection" folder exists
...
def calculate_initial_balance(cost_projections, assumed_target_roi):
    ...
    while upper_bound - lower_bound > tolerance:
        ...
        for year in range(total_years):
            ...
            # Apply the cash flow constraint for the first 5 years
            if year < 5:
...
# Set the years range from 2120 to 2180 (inclusive)
years = list(range(2120, 2181))

target_roi = Data_09_03_2025_CISF.targetROI
current_fund_value_2023 = Data_09_03_2025_CISF.currentFundValue2023

year_labels = []
needed_top_ups = []

for year in years:
    scenario_name = f"Conclusion_in_{year}"  # Expecting attributes like Conclusion_in_2120, etc.
    costs = getattr(Data_09_03_2025_CISF, scenario_name, None)
...
plot_path = get_next_filename("capital_injection_Scenario_3_CISF", "png", plot_dir)
...
plt.xticks(year_labels[::5], rotation=45)
plt.ylim(17.00, 35.00)  # Set y-axis range from 17 to 35 billion
plt.grid(True)
plt.legend()
